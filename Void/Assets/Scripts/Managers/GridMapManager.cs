...
//        facilityFloor = new FacilityFloor(seed, gridSize, Vector3.zero, tileSize, interiorTilesPerMapTile);
//    }
...
//        foreach (TileCollection collection in tileCollections)
//        {
//            Vector2Int offset = collection.Bounds.lowerLeft * interiorTilesPerMapTile;
//            Pathfinder2D pathfinder2D = new Pathfinder2D((collection.Bounds.upperRight - collection.Bounds.lowerLeft) * interiorTilesPerMapTile);

//            for (int i = 0; i < collection.connections.Count - 1; i++)
//            {
//                Prim.Edge edge = new Prim.Edge(new Vertex(collection.connections[i].from * interiorTilesPerMapTile - offset), new Vertex(collection.connections[i + 1].from * interiorTilesPerMapTile - offset));

//                Vector2Int start = new Vector2Int((int)edge.U.Position.x, (int)edge.U.Position.y);
//                Vector2Int end = new Vector2Int((int)edge.V.Position.x, (int)edge.V.Position.y);

//                InteriorTile startTile = interiorGridMap[start];
//                InteriorTile endTile = interiorGridMap[end];
...
//                    InteriorTile interiorTile = interiorGridMap[offset + b.Position];
...
//                if (path != null)
//                {
//                    ForcePlaceInteriorTiles(InteriorTile.InteriorTileType.Walkway, null, path);
//                    foreach (Vector2Int pos in path)
//                    {
//                        Vector2 newSpawnPosition = (((Vector2)pos) - (interiorTilesPerMapTile / 2) * Vector2.one) / interiorTilesPerMapTile;
//                        Instantiate(debugMarker, tileSize * new Vector3(newSpawnPosition.x, 5, newSpawnPosition.y), Quaternion.identity, levelHolder.transform);
//                    }
//                }
//            }
//        }
